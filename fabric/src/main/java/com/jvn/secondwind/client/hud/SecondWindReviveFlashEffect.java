...
public final class SecondWindReviveFlashEffect {
    ...
    private static void drawCenteredScaledString(
        ...
        graphics.drawString(font, visualText, Math.round(-textWidth / 2.0F), Math.round(-textHeight / 2.0F), color, false);
    ...
    private static void drawCenteredScaledOutlinedString(
        ...
        graphics.pose().mulPose(Axis.ZP.rotationDegrees(rotationDegrees));
        graphics.pose().scale(scale, scale, 1.0F);
        int textX = Math.round(-textWidth / 2.0F);
        int textY = Math.round(-textHeight / 2.0F);
        graphics.drawString(font, visualText, textX - outlineOffset, textY, outlineColor, false);
        graphics.drawString(font, visualText, textX + outlineOffset, textY, outlineColor, false);
        graphics.drawString(font, visualText, textX, textY - outlineOffset, outlineColor, false);
        graphics.drawString(font, visualText, textX, textY + outlineOffset, outlineColor, false);
        graphics.drawString(font, visualText, textX, textY, color, false);
        graphics.pose().popPose();
    }
